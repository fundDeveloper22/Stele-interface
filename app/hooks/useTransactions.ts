...
export function useTransactions(challengeId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      console.log('🔍 Fetching transactions for challengeId:', challengeId)
      
      try {
        // First, test if subgraph is accessible at all
        console.log('🧪 Testing subgraph connectivity...')
        try {
          const testData = await request(SUBGRAPH_URL, TEST_QUERY)
          console.log('✅ Subgraph connectivity test successful:', testData)
        } catch (testError) {
          console.error('🚫 Subgraph connectivity test failed:', testError)
          throw new Error(`Subgraph is not accessible: ${testError}`)
        }
        ...
        if (data.creates && Array.isArray(data.creates)) {
          data.creates.forEach((create) => {
            console.log('✨ Processing create:', create)
        ...
        if (data.swaps && Array.isArray(data.swaps)) {
          data.swaps.forEach((swap) => {
            console.log('🔄 Processing swap:', swap)
        ...
        if (data.registers && Array.isArray(data.registers)) {
          data.registers.forEach((register) => {
            console.log('📈 Processing register:', register)
            allTransactions.push({
              type: 'register',
              id: register.id,
        ...
        console.log('📋 Final transactions:', allTransactions)

        // If no transactions found for this challengeId, let's also try to fetch some general data
        if (allTransactions.length === 0) {
          console.log('🚨 No transactions found for challengeId:', challengeId)
          console.log('🔎 Fetching all transactions to see what data exists...')
          
          try {
            const allData = await request<GraphQLResponse>(SUBGRAPH_URL, GET_ALL_TRANSACTIONS_QUERY)
            console.log('📊 All transactions in subgraph:', allData)
            ...
            if (allData) {
              ...
              console.log('🆔 Available challengeIds in subgraph:', Array.from(availableChallengeIds))
            }
          } catch (debugError) {
